...
constexpr float kMinimumDrawableDimension = 1.0f;

std::tuple<float, float, float, float> getBoundsForDrawable(NSVGshape *shape, NSVGpath *path) {
    float minX = path->bounds[0];
    float minY = path->bounds[1];
    float maxX = path->bounds[2];
    float maxY = path->bounds[3];

    if (shape->stroke.type != NSVG_PAINT_NONE) {
        const float strokePadding = shape->strokeWidth * 0.5f;
        minX -= strokePadding;
        minY -= strokePadding;
        maxX += strokePadding;
        maxY += strokePadding;
    ...
    if (width < kMinimumDrawableDimension) {
        const float padding = (kMinimumDrawableDimension - width) * 0.5f;
        minX -= padding;
        maxX += padding;
        width = kMinimumDrawableDimension;
    }

    if (height < kMinimumDrawableDimension) {
        const float padding = (kMinimumDrawableDimension - height) * 0.5f;
        minY -= padding;
        maxY += padding;
        height = kMinimumDrawableDimension;
    }

    return {width, height, minX, minY};
}

void addLocalizedPath(TSP::Path *targetPath, NSVGpath *sourcePath, float originX, float originY) {
    addPathElement(targetPath, sourcePath->pts[0] - originX, sourcePath->pts[1] - originY, TSP::Path_ElementType_moveTo);

    for (int index = 0; index < sourcePath->npts - 1; index += 3) {
        float *point = &sourcePath->pts[index * 2];
        addBezierPathElement(
            targetPath,
            point[2] - originX, point[3] - originY,
            point[4] - originX, point[5] - originY,
            point[6] - originX, point[7] - originY
        );
    }

    if (sourcePath->closed) {
        addCloseSubpathElement(targetPath);
    }
}
...
void addEditablePathFromSVGToPasteboard(std::vector<MessageWrapper *> *messageList, struct NSVGimage *image, google::protobuf::uint64 &currentIdentifier) {
    for (auto* shape = image->shapes; shape != NULL; shape = shape->next) {
        ...
        for (auto* path = shape->paths; path != NULL; path = path->next) {
            ...
            auto [pathWidth, pathHeight, pathX, pathY] = getBoundsForDrawable(shape, path);

            auto* size = createSize(pathWidth, pathHeight);
            bezierPathSourceArchive->set_allocated_naturalsize(size);

            auto* bez_path = new TSP::Path;
            addLocalizedPath(bez_path, path, pathX, pathY);
            ...
            auto* shapeInfoArchive = createCustomBezierShapeInfoArchive(shapeInfoIdentifier, shapeStyleIdentifier, bezierPathSourceArchive, pathHeight, pathWidth, pathX, pathY);
            ...
            addDrawableReferenceToPasteboard(messageList, shapeInfoIdentifier, pathX, pathY, pathWidth, pathHeight);
